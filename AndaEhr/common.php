...
function initializeDatabase() {
    ...
    try {
        ...
        try {
            ...
            error_log("⚠️ Index creation warning: " . $e->getMessage());
        }
...
function getPendingVitals() {
    ...
    try {
        $sql = "SELECT c.*, p.full_name, p.patient_id as patient_uid
                FROM consultations c 
...
function updateDoctorOrders($consultation_id, $orders, $doctor_id) {
    ...
    try {
        ...
        $check_sql = "SELECT doctor_orders FROM consultations WHERE id = :consultation_id";
        $check_stmt = $pdo->prepare($check_sql);
        $check_stmt->execute([':consultation_id' => $consultation_id]);
        $existing_orders = $check_stmt->fetchColumn();
        ...
        if ($result) {
            $action = empty($existing_orders) ? 'added' : 'updated';
            error_log("✅ Doctor orders $action for consultation: " . $consultation_id);
        }
 ...
 */
function getConsultationById($consultation_id) {
    $pdo = getDBConnection();
    if (!$pdo) return false;
    
    try {
        $sql = "SELECT c.*, p.full_name as patient_name, p.patient_id as patient_uid,
                       u.full_name as doctor_name, u2.full_name as nurse_name
                FROM consultations c
                JOIN patients p ON c.patient_id = p.patient_id
                LEFT JOIN users u ON c.orders_issued_by = u.id
                LEFT JOIN users u2 ON c.vital_signs_recorded_by = u2.id
        ...
        error_log("❌ Get consultation by ID error: " . $e->getMessage());
        return false;
    }
}
